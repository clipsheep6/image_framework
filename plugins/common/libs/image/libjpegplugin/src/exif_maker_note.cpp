...
constexpr unsigned char ExifHeader[] = {'E', 'x', 'i', 'f', '\0', '\0'};
constexpr unsigned char hwMnoteHeader[] = { 'H', 'U', 'A', 'W', 'E', 'I', '\0', '\0' };
constexpr unsigned char hwMnoteTiffII[] = { 'I', 'I', 0x2A, 0x00, 0x08, 0x00, 0x00, 0x00 };
constexpr unsigned char hwMnoteTiffMM[] = { 'M', 'M', 0x00, 0x2A, 0x00, 0x00, 0x00, 0x80 };
constexpr unsigned char hwMnoteIfdTail[] = { 0x00, 0x00, 0x00, 0x00 };
...
uint32_t ExifMakerNote::Parser(ExifData *exif, const unsigned char *data, uint32_t size)
{
    ...
    if ((data == nullptr) || (size < sizeof(ExifHeader))) {
    ...
    const unsigned char *&newData, uint32_t &newSize)
{
    ...
    if (size < sizeof(ExifHeader)) {
        HiLog::Error(LABEL, "FindExifLocation, small. size=%{public}u", size);
        return false;
    }

    const unsigned char *d = data;
    if (memcmp(d, ExifHeader, sizeof(ExifHeader)) != 0) {
...
bool ExifMakerNote::IsHwMakerNote(const unsigned char *data, uint32_t size)
{
    ...
    if (sizeof(hwMnoteTiffII) != sizeof(hwMnoteTiffMM)) {
        HiLog::Fatal(LABEL, "IsHwMakerNote leave, same");
        return false;
    }
    if (size < (sizeof(hwMnoteHeader) + sizeof(hwMnoteTiffII))) {
        HiLog::Debug(LABEL, "IsHwMakerNote leave, size");
        return false;
    }

    if (memcmp(data, hwMnoteHeader, sizeof(hwMnoteHeader)) != 0) {
        HiLog::Debug(LABEL, "IsHwMakerNote leave, hd");
        return false;
    }

    tiff_offset_ = sizeof(hwMnoteHeader);

    if (memcmp((data + tiff_offset_), hwMnoteTiffII, sizeof(hwMnoteTiffII)) == 0) {
        order_ = ExifByteOrder::EXIF_BYTE_ORDER_INTEL;
        ifd0_offset_ = tiff_offset_ + sizeof(hwMnoteTiffII);
        HiLog::Debug(LABEL, "IsHwMakerNote leave, ii, tiff=%{public}u, ifd0=%{public}u", tiff_offset_, ifd0_offset_);
        return true;
    }

    if (memcmp((data+ tiff_offset_), hwMnoteTiffMM, sizeof(hwMnoteTiffMM)) == 0) {
        order_ = ExifByteOrder::EXIF_BYTE_ORDER_MOTOROLA;
        ifd0_offset_ = tiff_offset_ + sizeof(hwMnoteTiffMM);
...
bool ExifMakerNote::ParserIFD(uint32_t offset, uint32_t ifd, uint32_t deep)
{
    ...
    if (memcmp(makerNote_.data() + offset, hwMnoteIfdTail, sizeof(hwMnoteIfdTail)) != 0) {
