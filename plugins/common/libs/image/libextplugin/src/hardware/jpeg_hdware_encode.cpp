...
struct OfflineRequestSetting {
    ...
    std::vector<uint32_t> auxiliaryTypes;
};
...
std::vector<uint8_t> WriteSettings(const OfflineRequestSetting& inputSeetings, unsigned int tagId)
{
    ...
    metaData->addEntry("tag_bufferTypeArr", inputSeetings.auxiliaryTypes.data(), inputSeetings.auxiliaryTypes.size());
...
int32_t JpegHardwareEncoder::JpegEncodeImage(SkWStream* output, const PlEncodeOptions& opts, Media::Picture* picture)
{
    ...
    std::map<AuxiliaryPictureType, PlImageBuffer> auxiliaryMaps;
    std::map<AuxiliaryPictureType, OHOS::sptr<NativeBuffer>> auxiliaryNativeBuffers;
    ...
    for (auto type : auxiliaryTypes) {
        if (picture->HasAuxiliaryPicture(type)) {
            ...
            retCode = CopyInputBufferFromPixelMap(inputAuxiliaryBuffer, gainMapPixel.Get());
            if (retCode != SUCCESS) {
                IMAGE_LOGE("JpegEncodeImage CopyInputBufferFromPixelMap failed, "
                    "inputAuxiliaryBuffer is null  %{public}d", type);
                return INPUT_BUFFER_NULL;
            }
            auxiliaryMaps[type] = inputAuxiliaryBuffer;
            ...
            auxiliaryNativeBuffers[type] = nativeBuffer;
        }
    }
    ...
    int32_t outBufferSize = static_cast<int32_t>(inputSurfaceBuffer->GetSize());
    int index = 1;
    for (const auto &pair: auxiliaryNativeBuffers) {
        outBufferSize += static_cast<int32_t>(pair.second->GetSize());
    ...
    OfflineRequestSetting inputSettings;
    for (const auto &pair: auxiliaryNativeBuffers) {
        auto type = ConvertHardwareType(pair.first)
        inputSettings.auxiliaryTypes.push_back(type);
    }
    MediaOffline::V1_0::OfflineRequest req {
        .requestIndex = 0,
        .settings = WriteSettings(inputSettings, tagId),
    ...
    for (const auto &pair: auxiliaryMaps) {
        ReleaseDmaBuffer(pair.second);
    }
    ReleaseDmaBuffer(outputBuffer);
    return retCode;
}
