...
#define SUCCESS OHOS::Media::SUCCESS
        ...
        uint32_t ExtDecoder::HeifYUVMemAlloc(OHOS::ImagePlugin::DecodeContext &context)
        {
            ...
            GraphicPixelFormat graphicPixelFormat = context.info.pixelFormat == PlPixelFormat::NV12 ? GRAPHIC_PIXEL_FMT_YCBCR_420_SP : GRAPHIC_PIXEL_FMT_YCRCB_420_SP;
            sptr<SurfaceBuffer> hwBuffer = decoder.AllocateOutputBuffer(info_.width(), info_.height(), graphicPixelFormat);
            if (hwBuffer == nullptr)
            {
        ...
        bool ExtDecoder::IsSupportHardwareDecode()
        {
            ...
            if (!(ImageSystemProperties::GetHardWareDecodeEnabled() && codec_->getEncodedFormat() == SkEncodedImageFormat::kJPEG))
            {
                return false;
            }
            int width = info_.width();
            int height = info_.height();
            return width >= HARDWARE_MIN_DIM && width <= HARDWARE_MAX_DIM && height >= HARDWARE_MIN_DIM && height <= HARDWARE_MAX_DIM;
        }
        ...
        uint32_t ExtDecoder::DoHeifToYuvDecode(OHOS::ImagePlugin::DecodeContext &context)
        {
            ...
            decoder->setOutputColor(context.info.pixelFormat == PlPixelFormat::NV12 ? kHeifColorFormat_NV12 : kHeifColorFormat_NV21);
