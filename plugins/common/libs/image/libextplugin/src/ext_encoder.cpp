...
uint32_t ExtEncoder::EncodeHeifPicture(ExtWStream& outputStream)
{
    ...
    bool res = hwEncoder.AssembleOuputSharedBuffer(outputBuffer, outputMem, output_);
    if (!res) {
        return ERR_IMAGE_INVALID_PARAMETER;
    }
    ret = hwEncoder.Encode(picture_, opts_, outputBuffer);
    if (ret != SUCCESS) {
        hwEncoder.RecycleAllocatedResource();
        return ERR_IMAGE_ENCODE_FAILED;
    }
    if (output_->GetType() != OutputStreamType::FILE_PACKER) {
        // res = output_->Write(reinterpret_cast<uint8_t *>(outputMem->data.data), outputBuffer.fillenLen);
        res = outputStream.write(outputMem->data.data, outputBuffer.fillenLen);
    }
    hwEncoder.RecycleAllocatedResource();
    if (!res) {
        return ERR_IMAGE_ENCODE_FAILED;
