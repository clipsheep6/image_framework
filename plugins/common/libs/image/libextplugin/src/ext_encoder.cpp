...
static sptr<SurfaceBuffer> AllocSurfaceBuffer(SkImageInfo info, CM_HDR_Metadata_Type type, CM_ColorSpaceType color)
{
    ...
    MetadataHelper::SetHDRMetadataType(sb, type);
    MetadataHelper::SetColorSpaceType(sb, color);
    return sb;
#endif
}
...
static HdrMetadata GetHdrMetadata(sptr<SurfaceBuffer>& sb)
{
    ...
    MetadataHelper::GetHDRDynamicMetadata(sb, dynamicMetadata);
    std::vector<uint8_t> staticMetadata = {};
    MetadataHelper::GetHDRStaticMetadata(sb, staticMetadata);
    ...
    ImagePlugin::HdrMetadata& metadata)
{
    ...
    MetadataHelper::SetHDRMetadataType(hdrSurfaceBuffer, CM_IMAGE_HDR_VIVID_SINGLE);
    MetadataHelper::SetColorSpaceType(hdrSurfaceBuffer, CM_BT2020_HLG_LIMIT);
    MetadataHelper::SetHDRDynamicMetadata(hdrSurfaceBuffer, std::vector<uint8_t>());
    MetadataHelper::SetHDRStaticMetadata(hdrSurfaceBuffer, std::vector<uint8_t>());
    metadata = GetHdrMetadata(hdrSurfaceBuffer);
#ifdef IMAGE_HDR_CONVERTER_FLAG
    auto convert = Media::Video::ProcessingEngine::ColorSpaceConverter::Create();
    ColorSpaceConverterParameter param;
    param.renderIntent = RenderIntent::RENDER_INTENT_ABSOLUTE_COLORIMETRIC;
    convert->SetParameter(param);
...
uint32_t ExtEncoder::EncodeSingleVivid(SkEncodedImageFormat format, ExtWStream& outputStream)
{
    ...
    if (errorCode != SUCCESS) {
        IMAGE_LOGE("ExtEncoder::EncodeSingleVivid BuildSkBitmap failed");
        return errorCode;
    }
    if (!SkEncodeImage(&outputStream, bitmap, format, opts_.quality)) {
        IMAGE_LOGE("ExtEncoder::EncodeSingleVivid encode failed");
        return ERR_IMAGE_ENCODE_FAILED;
    }
    return SUCCESS;
}
    ...
    SkWStream& outputStream, HdrMetadata metadata)
{
    ...
    std::vector<uint8_t> metadataPack = HdrUtils::PackJpegVividMetadata(metadata);
    uint32_t gainMapSize = gainMapImage->size() + metadataPack.size();
    std::vector<uint8_t> baseInfo = HdrUtils::PackJpegVividBaseInfo(baseImage->size(), gainMapSize);
...
uint32_t ExtEncoder::EncodeSdrImage(SkEncodedImageFormat format, ExtWStream& outputStream)
{
    if (pixelmap_->GetPixelFormat() != Media::PixelFormat::RGBA_1010102) {
        ...
        auto errorCode = BuildSkBitmap(pixelmap_, bitmap, format, holder);
        if (errorCode != SUCCESS) {
            IMAGE_LOGE("ExtEncoder::EncodeSdrImage BuildSkBitmap failed");
            return errorCode;
        }
        if (!SkEncodeImage(&outputStream, bitmap, format, opts_.quality)) {
            IMAGE_LOGE("ExtEncoder::EncodeSdrImage encode failed");
            return ERR_IMAGE_ENCODE_FAILED;
        }
        return SUCCESS;
    }
