...
uint32_t ExtEncoder::FinalizeEncode()
{
    ...
    return EncodeImageByPixelMap(pixelmap_, opts_.needExif, wStream);
#else
...
uint32_t ExtEncoder::EncodeDualVivid(ExtWStream& outputStream)
{
    ...
    sk_sp<SkData> baseImageData = GetImageEncodeData(baseSptr, baseInfo, opts_.needExif);
...
{
    if (pixelmap_->GetPixelFormat() != PixelFormat::RGBA_1010102) {
        return EncodeImageByPixelMap(pixelmap_, opts_.needExif, outputStream);
    }
    ImageInfo info;
    ...
    error = EncodeImageBySurfaceBuffer(baseSptr, baseInfo, opts_.needExif, outputStream);
    FreeBaseAndGainMapSurfaceBuffer(baseSptr, gainMapSptr);
    return error;
}
#endif
