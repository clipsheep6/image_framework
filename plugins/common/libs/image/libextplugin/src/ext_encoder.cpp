...
int InitAstcencConfig(astcenc_profile profile, TextureEncodeOptions *option, astcenc_config& config)
{
    ...
    if (status == ASTCENC_ERR_BAD_BLOCK_SIZE) {
        HiLog::Debug(LABEL, "ERROR: block size is invalid");
        return 1;
    } else if (status == ASTCENC_ERR_BAD_CPU_FLOAT) {
        HiLog::Debug(LABEL, "ERROR: astcenc must not be compiled with fast-math");
        return 1;
    } else if (status != ASTCENC_SUCCESS) {
        HiLog::Debug(LABEL, "ERROR: config failed");
        return 1;
    }
    return 0;
}
...
uint32_t ExtEncoder::ASTCEncode()
{
    ImageInfo imageInfo;
    ...
    if (inRGBA == nullptr) {
        HiLog::Error(LABEL, "astc malloc failed");
        return ERROR;
    }
    ...
    errno_t ret = memcpy_s(output_->GetAddr(), sizeof(astc_header), &work.head, sizeof(astc_header));
    if (ret != 0) {
