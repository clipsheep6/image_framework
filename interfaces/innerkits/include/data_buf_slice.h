 ...
 *
 * The public methods are implemented using a storage_type. This type contains the actual reference to the data to
...
struct ConstSliceBase : SliceBase {
  ...
  /**
   * @brief Constructor, requires begin to be smaller than end, otherwise an exception is thrown.
   * Also forwards all parameters to the constructor of storage_
   *
  ...
  /**
   * @brief Obtain a constant reference to the element with the specified index in the slice.
   *
 ...
 */
template <typename container>
struct Slice : public MutableSliceBase<ContainerStorage, container> {
 ...
 */
template <typename container>
struct Slice<const container> : public ConstSliceBase<ContainerStorage, const container> {
  ...
  Slice subSlice(size_t begin, size_t end) const {
    return ConstSliceBase<ContainerStorage,
 ...
 */
template <typename T>
struct Slice<const T*> : public ConstSliceBase<PtrSliceStorage, const T*> {
  /*!
   * Constructor.
   *
  ...
  Slice(const T* ptr, size_t begin, size_t end) :
      ConstSliceBase<PtrSliceStorage, const T*>(ptr, begin, end) {
    // TODO: use using in C++11
  }

  Slice<const T*> subSlice(size_t begin, size_t end) const {
    return ConstSliceBase<PtrSliceStorage, const T*>::template subSlice<Slice<const T*>>(begin,
                                                                                                             end);
  }
};
 ...
 */
template <typename T>
struct Slice<T*> : public MutableSliceBase<PtrSliceStorage, T*> {
