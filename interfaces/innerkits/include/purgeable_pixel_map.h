...
#ifndef INTERFACES_INNERKITS_INCLUDE_PURGEABLE_PIXEL_MAP_H_
#define INTERFACES_INNERKITS_INCLUDE_PURGEABLE_PIXEL_MAP_H_
...
namespace OHOS {
namespace Media {
using CustomFreePurgeablePixelMap = void (*)(void *addr, void *context, uint32_t size);

class PurgeablePixelMap : public Parcelable {
public:
    PurgeablePixelMap() = default;
    virtual ~PurgeablePixelMap();
    NATIVEEXPORT static std::unique_ptr<PurgeablePixelMap> Create(const uint32_t *colors, uint32_t colorLength,
                                                         const InitializationOptions &opts);
    NATIVEEXPORT static std::unique_ptr<PurgeablePixelMap> Create(const uint32_t *colors, uint32_t colorLength, int32_t offset,
                                                         int32_t stride, const InitializationOptions &opts);
    NATIVEEXPORT static std::unique_ptr<PurgeablePixelMap> Create(const InitializationOptions &opts);
    // NATIVEEXPORT static std::unique_ptr<PurgeablePixelMap> Create(PurgeablePixelMap &source, const InitializationOptions &opts);
    // NATIVEEXPORT static std::unique_ptr<PurgeablePixelMap> Create(PurgeablePixelMap &source, const Rect &srcRect,
    //                                                      const InitializationOptions &opts);
    ...
    NATIVEEXPORT void SetPixelsAddr(void *addr, void *context, uint32_t size, AllocatorType type,
                                    CustomFreePurgeablePixelMap func);
    ...
    NATIVEEXPORT bool IsSameImage(const PurgeablePixelMap &other);
    ...
    NATIVEEXPORT void FreePurgeablePixelMap();
    NATIVEEXPORT AllocatorType GetAllocatorType();
    NATIVEEXPORT void *GetFd() const;

    NATIVEEXPORT bool BeginRead();
    NATIVEEXPORT void EndRead();
    NATIVEEXPORT bool BeginWrite();
    NATIVEEXPORT void EndWrite();
    ...
    {
        return static_cast<void *>(data_->GetContent());
    }

    NATIVEEXPORT bool Marshalling(Parcel &data) const override;
    // NATIVEEXPORT static PurgeablePixelMap *Unmarshalling(Parcel &data);
...
private:
    ...
    // static bool CopyPurgeablePixelMap(PurgeablePixelMap &source, PurgeablePixelMap &dstPixelMap);
    // static bool SourceCropAndConvert(PurgeablePixelMap &source, const ImageInfo &srcImageInfo, const ImageInfo &dstImageInfo,
    //                                  const Rect &srcRect, PurgeablePixelMap &dstPixelMap);
    static bool IsSameSize(const Size &src, const Size &dst);
    // static bool ScalePurgeablePixelMap(const Size &targetSize, const Size &dstSize, const ScaleMode &scaleMode,
    //                           PurgeablePixelMap &dstPixelMap);
    ...
    PurgeableMem::PurgeableMem *data_ = nullptr;
    ...
    CustomFreePurgeablePixelMap custFreePurgeablePixelMap_ = nullptr;
...
#endif // INTERFACES_INNERKITS_INCLUDE_PURGEABLE_PIXEL_MAP_H_
