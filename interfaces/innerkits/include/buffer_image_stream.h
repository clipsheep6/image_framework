...
class BufferImageStream : public ImageStream {
  public:
     ...
     *
     * Initially, when the write range does not exceed originData, the memory
     * uses the pointer of originData, and there will be no memory copy.
     * However, when the write range exceeds originData, it will choose
     * according to the MemoryMode. If the mode is Fix, it will report a write
     * error and keep the original pointer unchanged. If the mode is Dynamic, it
     * will reallocate memory, copy the original data, and then write new data.
     *
     ...
     */
    NATIVEEXPORT BufferImageStream(byte *originData, size_t size,
                                   MemoryMode mode);
     ...
     * as the data for BufferImageStream is already in memory and there is no
     * need to flush.
     *
     ...
     * Since the data of BufferImageStream is already in memory, this function
     * directly returns the pointer to the data. Note that this function ignores
     * the isWriteable parameter, because the data of BufferImageStream is
     * always writable.
     *
    ...
    /**
     * After calling Release, BufferImageStream no longer manages the lifecycle
     * of the pointer. The lifecycle of the pointer needs to be managed by the
     * user. Please note that if the returned pointer is the one passed in the
     * constructor, then the lifecycle of this pointer should be managed by the
     * one who created it. If the pointer was created by BufferImageStream, then
     * the lifecycle should be managed by the one who calls Release.
     *
