...
bool JpgYuvTest::ReadFile(void *ch_org, std::string path, int32_t totalsize, int32_t src_num)
{
...
void JpgYuvTest::RGBA1010102Crop(PixelFormat outfmt, std::string outname, ImageSize &imageSize)
{
    GTEST_LOG_(INFO) << "JpgYuvTest: request size(" << imageSize.width << ", " << imageSize.height << ")";
    int32_t y_size = imageSize.width * imageSize.height;
    const size_t total_size = y_size * NUM_4;
    uint8_t* const ch_org = new uint8_t[total_size];
    bool result = ReadFile(ch_org, IMAGE_INPUT_YUV_PATH4, total_size, 0);
    ASSERT_EQ(result, true);
...
void JpgYuvTest::RGBA1010102Scale(PixelFormat outfmt, std::string &outname, ImageSize &imageSize, AntiAliasingOption option)
{
    GTEST_LOG_(INFO) << "JpgYuvTest: request size(" << imageSize.width << ", " << imageSize.height << ")";
    int32_t y_size = imageSize.width * imageSize.height;
    const size_t total_size = y_size * NUM_4;
...
void JpgYuvTest::RGBA1010102Translate(PixelFormat outfmt, std::string &outname, ImageSize &imageSize, Coordinate &coordinate)
{
    GTEST_LOG_(INFO) << "JpgYuvTest: request size(" << imageSize.width << ", " << imageSize.height << ")";
    int32_t y_size = imageSize.width * imageSize.height;
    const size_t total_size = y_size * NUM_4;
