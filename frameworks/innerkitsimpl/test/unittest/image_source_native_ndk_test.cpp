...
static constexpr int32_t BufferLength = 8;
/**
...
{
    uint8_t color[BufferLength] = {0x80, 0x02, 0x04, 0x08, 0x40, 0x02, 0x04, 0x08};
    size_t dataLength = BufferLength;
    OH_ImageSourceNative *source = nullptr;
    OH_DecodingOptionsForPicture *options = nullptr;
    OH_PictureNative *picture = nullptr;

    Image_ErrorCode ret = OH_ImageSourceNative_CreateFromData(color, dataLength, &source);
    EXPECT_EQ(ret, IMAGE_SUCCESS);
