...
std::string MODIFYDATA[][3] = {
    ...
    {"GPSTimeStamp", "11/1 37/1 56/1", "11:37:58"},
    ...
    // {"SceneType", "1", "(unknown value 49)"},
    // {"SensitivityType", "5", "5"},
    // {"StandardOutputSensitivity", "5", "Standard output sensitivity (SOS) and ISO speed"},
...
HWTEST_F(ExifMetadataTest, SetValue_001, TestSize.Level3)
{
    ...
    for (int i = 0; i < rows; ++i) {
        ...
        std::string errorstr;
        int32_t iret = metadata.SetValue_(key, modifyvalue, errorstr);
        std::string retvalue;
        metadata.GetValue(key, retvalue);

        GTEST_LOG_(INFO) << "ExifMetadataTest: SetValue_001" << "key: " << key << " modifyvalue: " << modifyvalue 
            << " iret: " << iret << " errorstr: " << errorstr << " retvalue: " << retvalue;
        // ASSERT_EQ(retvalue, modifyvalue);
