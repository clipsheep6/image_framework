...
HWTEST_F(PixelMapTest, ConvertAlphaFormatTest001, TestSize.Level3)
{
    ...
    pixelMap2->GetImageInfo(dstImageInfo);
    void *pixelMapData = pixelMap2->GetWritablePixels();
    uint32_t ret = pixelMap1->ConvertAlphaFormat(dstImageInfo, pixelMap2->GetCapacity(), pixelMap2->GetPixelBytes(),
                                                    pixelMapData, true);
    ASSERT_NE(ret, SUCCESS);

    ret = pixelMap1->ConvertAlphaFormat(dstImageInfo, pixelMap2->GetCapacity(), pixelMap2->GetPixelBytes(),
                                        pixelMapData, false);
    ASSERT_NE(ret, SUCCESS);
...
HWTEST_F(PixelMapTest, ConvertAlphaFormatTest003, TestSize.Level3)
{
    ...
    EXPECT_TRUE(pixelMap3 != nullptr);
    pixelMap3->GetImageInfo(dstImageInfo);
    void *pixelMapData3 = pixelMap3->GetWritablePixels();
    uint8_t *wpixel3 = static_cast<uint8_t *>(pixelMapData3);
    ret = pixelMap2->ConvertAlphaFormat(dstImageInfo, pixelMap3->GetCapacity(), pixelMap3->GetPixelBytes(),
                                        pixelMapData3, false);
    ASSERT_EQ(ret, SUCCESS);
    for (int i = 0; i < colorLength; i += 4)
    {
