...
/**
 * @tc.name: IsSupportCropOnDecodeTest002
 * @tc.desc: Test of IsSupportCropOnDecode
 * @tc.type: FUNC
 */
HWTEST_F(ExtDecoderTest, IsSupportCropOnDecodeTest002, TestSize.Level3)
{
    GTEST_LOG_(INFO) << "ExtDecoderTest: IsSupportCropOnDecodeTest002 start";
    std::shared_ptr<ExtDecoder> extDecoder = std::make_shared<ExtDecoder>();
    SkIRect target;
    extDecoder->codec_ = nullptr;
    bool ret = extDecoder->IsSupportCropOnDecode(target);
    ASSERT_EQ(ret, false);
    GTEST_LOG_(INFO) << "ExtDecoderTest: IsSupportCropOnDecodeTest002 end";
}
 ...
 * @tc.type: FUNC
 */
HWTEST_F(ExtDecoderTest, ModifyImagePropertyTest002, TestSize.Level3)
{
    GTEST_LOG_(INFO) << "ExtDecoderTest: ModifyImagePropertyTest002 start";
    std::shared_ptr<ExtDecoder> extDecoder = std::make_shared<ExtDecoder>();
    uint32_t index = 0;
    std::string key = "Path";
    std::string value = "void";
    int fd = 0;
    uint32_t ret = extDecoder->ModifyImageProperty(index, key, value, fd);
    ASSERT_EQ(ret, Media::ERR_IMAGE_DECODE_EXIF_UNSUPPORT);
    GTEST_LOG_(INFO) << "ExtDecoderTest: ModifyImagePropertyTest002 end";
}
 ...
 * @tc.type: FUNC
 */
HWTEST_F(ExtDecoderTest, ModifyImagePropertyTest003, TestSize.Level3)
{
    GTEST_LOG_(INFO) << "ExtDecoderTest: ModifyImagePropertyTest003 start";
    std::shared_ptr<ExtDecoder> extDecoder = std::make_shared<ExtDecoder>();
    uint32_t index = 0;
    std::string key = "Path";
    std::string value = "void";
    ...
    uint32_t size = 0;
    uint32_t ret = extDecoder->ModifyImageProperty(index, key, value, data, size);
    ASSERT_EQ(ret, Media::ERR_IMAGE_DECODE_EXIF_UNSUPPORT);
    GTEST_LOG_(INFO) << "ExtDecoderTest: ModifyImagePropertyTest003 end";
}
