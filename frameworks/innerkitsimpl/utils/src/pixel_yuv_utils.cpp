...
bool PixelYuvUtils::YuvReversal(uint8_t *srcData, YuvImageInfo &srcInfo, uint8_t *dstData, YuvImageInfo &dstInfo)
{
    ...
    if (IsYUVP010Format(srcInfo.yuvFormat)) {
        tmpData = std::make_unique<uint8_t[]>(dataSize * 2);
    } else {
...
static void P010WritePixels(const YUVDataInfo &yuvInfo, uint16_t *srcPixels,
    const uint32_t &color, bool isNV12_P010, ImageInfo &info)
{
    ...
    const uint32_t &color)
{
    switch (info.pixelFormat) {
        ...
        case PixelFormat::YCBCR_P010:
        case PixelFormat::YCRCB_P010: {
            bool isNV12_P010 = (info.pixelFormat == PixelFormat::YCBCR_P010) ? true : false;
            P010WritePixels(yuvInfo, (uint16_t *)srcPixels, color, isNV12_P010, info);
            return true;
        }
        default:
            return false;
...
static void P010WritePixel(uint16_t *srcPixels, const YUVDataInfo &yuvDataInfo, const Position &pos,
    const uint32_t &color, bool isYCBCR_P010)
{
    ...
    const Position &pos, const uint32_t &color)
{
    switch (format) {
        ...
        case PixelFormat::YCRCB_P010:
        case PixelFormat::YCBCR_P010: {
            bool isYCBCR_P010 = (format == PixelFormat::YCBCR_P010) ? true : false;
            P010WritePixel((uint16_t *)srcPixels, yuvDataInfo, pos, color, isYCBCR_P010);
            return true;
        }
        default:
            return false;
