...
static bool NV12P010Rotate(uint8_t* src, PixelSize& size, YUVDataInfo& info, OpenSourceLibyuv::RotationMode& rotateNum)
{
    ...
    if (converter.P010ToI010(srcY, info.yWidth,srcUV, info.uvWidth * 2, dstY, info.yWidth, dstU, info.uvWidth,
    ...
    if (converter.I010ToP010(rotateY, info.yWidth, rotateU, info.uvWidth, rotateV, info.uvWidth, srcbuffer,
        GetUStride(info.yWidth), srcbuffer + GetYSize(size.dstW, size.dstH), GetUStride(info.yWidth), size.dstW,
        size.dstH) == -1) {
...
static void ScaleP010(int32_t dstYWidth, int32_t dstYHeight, YuvImageInfo &yuvInfo,
    uint8_t *src, uint8_t *dst, OpenSourceLibyuv::ImageConverter &converter, OpenSourceLibyuv::FilterMode &filterMode)
{
    uint16_t *srcBuffer = reinterpret_cast<uint16_t *>(src);
    uint16_t *dstBuffer = reinterpret_cast<uint16_t *>(dst);
    std::unique_ptr<uint16_t[]> dstPixels = std::make_unique<uint16_t[]>(GetImageSize(yuvInfo.width, yuvInfo.height));
    uint16_t* dstY = dstPixels.get();
    uint16_t* dstU = dstPixels.get() + GetYSize(yuvInfo.width, yuvInfo.height);
    uint16_t* dstV = dstPixels.get() + GetVOffset(yuvInfo.width, yuvInfo.height);
    if (converter.P010ToI010(srcBuffer, yuvInfo.width, srcBuffer + GetYSize(yuvInfo.width, yuvInfo.height),
        GetUVStride(yuvInfo.width), dstY, yuvInfo.width, dstU, GetUStride(yuvInfo.width), dstV,
        GetUStride(yuvInfo.width), yuvInfo.width, yuvInfo.height) == -1) {
        IMAGE_LOGE("NV12P010ToI010 failed");
        return;
    }
    std::unique_ptr<uint16_t[]> scalePixels = std::make_unique<uint16_t[]>(GetImageSize(dstYWidth, dstYHeight));
    uint16_t* scaleY = scalePixels.get();
    uint16_t* scaleU = scalePixels.get() + GetYSize(dstYWidth, dstYHeight);
    uint16_t* scaleV = scalePixels.get() + GetVOffset(dstYWidth, dstYHeight);
    if (converter.I420Scale_16(dstY, yuvInfo.width, dstU, GetUStride(yuvInfo.width), dstV,
        GetUStride(yuvInfo.width), yuvInfo.width, yuvInfo.height, scaleY, dstYWidth, scaleU,
        GetUStride(dstYWidth), scaleV, GetUStride(dstYWidth), dstYWidth, dstYHeight, filterMode) == -1) {
        IMAGE_LOGE("I420Scale_16 failed");
        return;
    }
    if (converter.I010ToP010(scaleY, dstYWidth, scaleU, GetUStride(dstYWidth), scaleV, GetUStride(dstYWidth),
        dstBuffer, GetUStride(dstYWidth), dstBuffer + GetYSize(dstYWidth, dstYHeight), GetUStride(dstYWidth),
        dstYWidth, dstYHeight) == -1) {
        IMAGE_LOGE("I010ToP010 failed");
        return;
    }
}
    ...
    YuvImageInfo &yuvInfo, uint8_t *src, uint8_t *dst)
{
    ...
    if (yuvInfo.yuvFormat == PixelFormat::YCBCR_P010 || yuvInfo.yuvFormat == PixelFormat::YCRCB_P010) {
        ScaleP010(dstYWidth, dstYHeight, yuvInfo, src, dst, converter, filterMode);
    } else {
