...
unique_ptr<FileSourceStream> FileSourceStream::CreateSourceStream(const int fd)
{
    ...
    IMAGE_LOGE("[FileSourceStream]fdopen, errno:%{public}d,%{public}s",errno, strerror(errno));
    if (filePtr == nullptr) {
        IMAGE_LOGE("[FileSourceStream]open file fail.");
        return nullptr;
    }
    int64_t offset = ftell(filePtr);
    IMAGE_LOGE("[FileSourceStream]ftell, errno:%{public}d,%{public}s",errno, strerror(errno));
    if (offset < 0) {
        IMAGE_LOGE("[FileSourceStream]get the position fail.");
        fclose(filePtr);
        return nullptr;
...
{
    IMAGE_LOGE("[FileSourceStream]Peek2, errno:%{public}d,%{public}s",errno, strerror(errno));
    if (desiredSize == 0 || filePtr_ == nullptr) {
    ...
    IMAGE_LOGE("[FileSourceStream]fseek2, errno:%{public}d,%{public}s",errno, strerror(errno));
    if (ret != 0) {
...
bool FileSourceStream::Peek(uint32_t desiredSize, uint8_t *outBuffer, uint32_t bufferSize, uint32_t &readSize)
{
    ...
    IMAGE_LOGE("[FileSourceStream]Peek1, errno:%{public}d,%{public}s",errno, strerror(errno));
    if (!GetData(desiredSize, outBuffer, bufferSize, readSize)) {
        IMAGE_LOGE("[FileSourceStream]peek fail.");
        return false;
    }
    int ret = fseek(filePtr_, fileOffset_, SEEK_SET);
    IMAGE_LOGE("[FileSourceStream]fseek, errno:%{public}d,%{public}s",errno, strerror(errno));
    if (ret != 0) {
        IMAGE_LOGE("[FileSourceStream]go to original position fail, ret:%{public}d.", ret);
        return false;
    }
...
{
    IMAGE_LOGE("[FileSourceStream]GetData1, errno:%{public}d,%{public}s",errno, strerror(errno));
    ...
    IMAGE_LOGE("[FileSourceStream]fread1, errno:%{public}d,%{public}s",errno, strerror(errno));
...
{
    IMAGE_LOGE("[FileSourceStream]GetData, errno:%{public}d,%{public}s",errno, strerror(errno));
    if (fileSize_ == fileOffset_) {
        IMAGE_LOGE("[FileSourceStream]read finish, offset:%{public}zu ,dataSize%{public}zu.", fileOffset_, fileSize_);
        return false;
    }
    ...
    ResetReadBuffer();
    IMAGE_LOGE("[FileSourceStream]ResetReadBuffer, errno:%{public}d,%{public}s",errno, strerror(errno));
    ...
    IMAGE_LOGE("[FileSourceStream]fread, errno:%{public}d,%{public}s",errno, strerror(errno));
    if (bytesRead < desiredSize) {
        IMAGE_LOGE("[FileSourceStream]read fail, bytesRead:%{public}zu", bytesRead);
        free(readBuffer_);
        readBuffer_ = nullptr;
