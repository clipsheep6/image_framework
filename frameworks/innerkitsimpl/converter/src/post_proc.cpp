...
uint32_t PostProc::DecodePostProc(const DecodeOptions &opts, PixelMap &pixelMap, FinalOutputStep finalOutputStep)
{
    ...
    dstImageInfo.pixelmapUniqueId_ = pixelMap->GetUniqueId();
...
uint32_t PostProc::AllocBuffer(ImageInfo imageInfo, uint8_t **resultData, uint64_t &bufferSize, int &fd, uint32_t uniqueId)
{
    ...
    if (decodeOpts_.allocatorType == AllocatorType::SHARE_MEM_ALLOC) {
        *resultData = AllocSharedMemory(imageInfo.size, bufferSize, fd, uniqueId);
        if (*resultData == nullptr) {
            IMAGE_LOGE("[PostProc]AllocSharedMemory failed");
            return ERR_IMAGE_CROP;
        }
...
bool PostProc::Transform(BasicTransformer &trans, const PixmapInfo &input, PixelMap &pixelMap)
{
    ...
    uint32_t ret;
    if (decodeOpts_.allocatorType == AllocatorType::SHARE_MEM_ALLOC) {
        typedef uint8_t *(*AllocMemory)(ImageInfo imageInfo, const uint64_t bufferSize, int &fd, uint32_t uniqueId);
