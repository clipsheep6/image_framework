...
bool ImageFormatConvertExtUtils::RGB565ToNV12P010(const uint8_t *srcBuffer, const Size &imageSize, uint8_t **destBuffer,
    size_t &destBufferSize, [[maybe_unused]] ColorSpace colorSpace)
{
    if (srcBuffer == nullptr || destBuffer == nullptr || imageSize.width < 0 || imageSize.height < 0) {
        IMAGE_LOGE("Input parameters not compliant!");
    ...
    *destBuffer = new (std::nothrow) uint8_t[destBufferSize]();
    ...
    converter.I010ToP010(I010Buffer, imageSize.width, I010Buffer + imageSize.width * imageSize.height,
        ...
        I010Buffer + imageSize.width * imageSize.height +
        ((imageSize.width + NUM_1) / NUM_2) * ((imageSize.height + NUM_1) / NUM_2),
        (imageSize.width + NUM_1) / NUM_2, P010Buffer, imageSize.width,
        P010Buffer + imageSize.width * imageSize.height, ((imageSize.width + NUM_1) / NUM_2) * NUM_2, imageSize.width,
        imageSize.height);
...
bool ImageFormatConvertExtUtils::RGB565ToNV21P010(const uint8_t *srcBuffer, const Size &imageSize, uint8_t **destBuffer,
    size_t &destBufferSize, [[maybe_unused]] ColorSpace colorSpace)
{
    if (srcBuffer == nullptr || destBuffer == nullptr || imageSize.width < 0 || imageSize.height < 0) {
        IMAGE_LOGE("Input parameters not compliant!");
    ...
    *destBuffer = new (std::nothrow) uint8_t[destBufferSize]();
    ...
    uint16_t *P010Buffer = reinterpret_cast<uint16_t *>(*destBuffer);
    converter.I010ToP010(I010Buffer, imageSize.width,
        I010Buffer + imageSize.width * imageSize.height +
        ((imageSize.width + NUM_1) / NUM_2) * ((imageSize.height + NUM_1) / NUM_2),
    ...
    size_t &destBufferSize, [[maybe_unused]] ColorSpace colorSpace)
{
    ...
    *destBuffer = new (std::nothrow) uint8_t[destBufferSize]();
    ...
    auto &converter = ConverterHandle::GetInstance().GetHandle();
    converter.ARGBToI420(srcBuffer, imageSize.width * NUM_4, i420Y, imageSize.width,
        i420U, (imageSize.width + NUM_1) / NUM_2, i420V, (imageSize.width + NUM_1) / NUM_2,
...
bool ImageFormatConvertExtUtils::RGBToNV21P010(const uint8_t *srcBuffer, const Size &imageSize, uint8_t **destBuffer,
    size_t &destBufferSize, [[maybe_unused]] ColorSpace colorSpace)
{
    if (srcBuffer == nullptr || destBuffer == nullptr || imageSize.width < NUM_0 || imageSize.height < NUM_0) {
        return false;
...
bool ImageFormatConvertExtUtils::RGBA1010102ToNV12(const uint8_t *srcBuffer, const Size &imageSize, uint8_t **destBuffer,
    size_t &destBufferSize, [[maybe_unused]] ColorSpace colorSpace)
{
    if (srcBuffer == nullptr || destBuffer == nullptr || imageSize.width < NUM_0 || imageSize.height < NUM_0) {
        return false;
    ...
    *destBuffer = new (std::nothrow) uint8_t[destBufferSize]();
    ...
    uint8_t *i420Buffer = new (std::nothrow) uint8_t[destBufferSize];
    if (i420Buffer == nullptr) {
        IMAGE_LOGE("apply space for dest buffer failed!");
        return false;
    }
...
bool ImageFormatConvertExtUtils::RGBA1010102ToNV21(const uint8_t *srcBuffer, const Size &imageSize, uint8_t **destBuffer,
    size_t &destBufferSize, [[maybe_unused]] ColorSpace colorSpace)
{
    if (srcBuffer == nullptr || destBuffer == nullptr || imageSize.width < NUM_0 || imageSize.height < NUM_0) {
        return false;
...
static bool NV12ToRGBA1010102NoManual(const uint8_t *srcBuffer, const YUVDataInfo &yDInfo, uint8_t **destBuffer,
    ColorSpace colorSpace)
{
    uint32_t p010BufferSize = yDInfo.yWidth * yDInfo.yHeight + yDInfo.uvWidth * EVEN_ODD_DIVISOR * yDInfo.uvHeight;
    uint16_t *p010Buffer = new (std::nothrow) uint16_t[p010BufferSize];
    if (p010Buffer == nullptr) {
        IMAGE_LOGE("Dynamically allocating memory for p010 buffer failed!");
        return false;
    }
    auto &converter = ConverterHandle::GetInstance().GetHandle();
    int32_t ret = converter.NV12ToP010(srcBuffer + yDInfo.yOffset, yDInfo.yStride, srcBuffer + yDInfo.uvOffset, yDInfo.uvStride,
        p010Buffer, yDInfo.yStride, p010Buffer + yDInfo.uvOffset, yDInfo.uvStride, yDInfo.yWidth, yDInfo.yHeight);
    if (ret != 0) {
        IMAGE_LOGE("NV12ToP010 failed, ret = %{public}d!", ret);
        delete[] p010Buffer;
        return false;
    }
    uint16_t *i010Buffer = new (std::nothrow) uint16_t[p010BufferSize];
    if (i010Buffer == nullptr) {
        IMAGE_LOGE("Dynamically allocating memory for i010 buffer failed!");
        delete[] p010Buffer;
        return false;
    }
    ret = converter.P010ToI010(p010Buffer, yDInfo.yStride, p010Buffer + yDInfo.uvOffset, yDInfo.uvStride,
               ...
               yDInfo.yWidth, yDInfo.yHeight);
    if (ret != 0) {
        IMAGE_LOGE("P010ToI010 failed, ret = %{public}d!", ret);
        delete[] p010Buffer;
        delete[] i010Buffer;
        return false;
    }
    ...
    ret = converter.I010ToAB30(i010Buffer, yDInfo.yWidth, i010Buffer + yDInfo.yWidth * yDInfo.yHeight, yDInfo.uvWidth,
               i010Buffer + yDInfo.yWidth * yDInfo.yHeight + yDInfo.uvWidth * yDInfo.uvHeight, yDInfo.uvWidth,
               *destBuffer, rgbaStride, yDInfo.yWidth, yDInfo.yHeight);
    if (ret != 0) {
    ...
    ColorSpace colorSpace)
{
    ...
    uint32_t rgbaStride = yDInfo.yWidth * BYTES_PER_PIXEL_RGBA;
    ret = converter.I010ToAB30(i010Buffer, yDInfo.yWidth, i010Buffer + yDInfo.yWidth * yDInfo.yHeight +
        yDInfo.uvWidth * yDInfo.uvHeight, yDInfo.uvWidth, i010Buffer + yDInfo.yWidth * yDInfo.yHeight, yDInfo.uvWidth,
        *destBuffer, rgbaStride, yDInfo.yWidth, yDInfo.yHeight);
    if (ret != 0) {
        IMAGE_LOGE("I010ToAB30 failed, ret = %{public}d!", ret);
        delete[] p010Buffer;
    ...
    ColorSpace colorSpace)
{
    ...
    I420Buffer = new (std::nothrow) uint8_t[yDInfo.yWidth * yDInfo.yHeight + yDInfo.uvWidth * EVEN_ODD_DIVISOR * yDInfo.uvHeight]();
    if (I420Buffer == nullptr) {
        IMAGE_LOGE("apply space for I420 buffer failed!");
        return false;
    }
    ...
    converter.NV12ToI420(nv12Y, yDInfo.yWidth, nv12UV, yDInfo.uvWidth * 2, i420Y, yDInfo.yWidth, i420U,
    ...
    ColorSpace colorSpace)
{
    ...
    converter.P010ToI010(src, yDInfo.yWidth, src + yDInfo.yWidth * yDInfo.yHeight, yDInfo.uvWidth * NUM_2,
        ...
        yDInfo.yWidth * yDInfo.yHeight + yDInfo.uvWidth * yDInfo.uvHeight, yDInfo.uvWidth, yDInfo.yWidth, yDInfo.yHeight);
    ...
    uint8_t **destBuffer, size_t &destBufferSize, [[maybe_unused]] ColorSpace colorSpace)
{
    ...
    *destBuffer = new (std::nothrow) uint8_t[destBufferSize]();
    ...
    int32_t result = converter.I420ToRGB565(i420Y, yDInfo.yWidth, i420U, yDInfo.uvWidth, i420V, yDInfo.uvWidth,
        *destBuffer, dstStrideRGB565, yDInfo.yWidth, yDInfo.yHeight);
    if (result) {
...
bool ImageFormatConvertExtUtils::NV12P010ToRGBA8888(const uint8_t *data, const YUVDataInfo &yDInfo, uint8_t **destBuffer,
    size_t &destBufferSize, [[maybe_unused]] ColorSpace colorSpace)
{
    if (data == nullptr || destBuffer == nullptr) {
        IMAGE_LOGE("Input buffer pointer data or destBuffer is null!");
...
bool ImageFormatConvertExtUtils::NV12P010ToBGRA8888(const uint8_t *data, const YUVDataInfo &yDInfo, uint8_t **destBuffer,
    size_t &destBufferSize, [[maybe_unused]] ColorSpace colorSpace)
{
    if (data == nullptr || destBuffer == nullptr) {
        IMAGE_LOGE("Input buffer pointer data or destBuffer is null!");
...
bool ImageFormatConvertExtUtils::NV12P010ToRGBA1010102(const uint8_t *data, const YUVDataInfo &yDInfo, uint8_t **destBuffer,
    size_t &destBufferSize, [[maybe_unused]] ColorSpace colorSpace)
{
    if (data == nullptr || destBuffer == nullptr) {
        IMAGE_LOGE("Input buffer pointer data or destBuffer is null!");
...
bool ImageFormatConvertExtUtils::NV21P010ToRGBA8888(const uint8_t *data, const YUVDataInfo &yDInfo, uint8_t **destBuffer,
    size_t &destBufferSize, [[maybe_unused]] ColorSpace colorSpace)
{
    if (data == nullptr || destBuffer == nullptr) {
        IMAGE_LOGE("Input buffer pointer data or destBuffer is null!");
...
bool ImageFormatConvertExtUtils::NV21P010ToBGRA8888(const uint8_t *data, const YUVDataInfo &yDInfo, uint8_t **destBuffer,
    size_t &destBufferSize, [[maybe_unused]] ColorSpace colorSpace)
{
    if (data == nullptr || destBuffer == nullptr) {
        IMAGE_LOGE("Input buffer pointer data or destBuffer is null!");
...
bool ImageFormatConvertExtUtils::NV21P010ToRGBA1010102(const uint8_t *data, const YUVDataInfo &yDInfo, uint8_t **destBuffer,
    size_t &destBufferSize, [[maybe_unused]] ColorSpace colorSpace)
{
    if (data == nullptr || destBuffer == nullptr) {
        IMAGE_LOGE("Input buffer pointer data or destBuffer is null!");
