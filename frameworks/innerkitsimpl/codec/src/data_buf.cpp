 ...
 */
template <typename... T>
constexpr void enforce(bool condition, ErrorCode err_code, T&&... args) {
  enforce<Error>(condition, err_code, std::forward<T>(args)...);
}
...
uint16_t DataBuf::read_uint16(size_t offset, ByteOrder byteOrder) const {
  if (pData_.size() < 2 || offset > (pData_.size() - 2)) {
    ...
    return 0;
    // throw std::out_of_range("Overflow in DataBuf::read_uint16");
  }
...
void DataBuf::write_uint16(size_t offset, uint16_t x, ByteOrder byteOrder) {
  if (pData_.size() < 2 || offset > (pData_.size() - 2)) {
    ...
    return;
    // throw std::out_of_range("Overflow in DataBuf::write_uint16");
  }
...
uint32_t DataBuf::read_uint32(size_t offset, ByteOrder byteOrder) const {
  if (pData_.size() < 4 || offset > (pData_.size() - 4)) {
    ...
    return 0;
    // throw std::out_of_range("Overflow in DataBuf::read_uint32");
  }
...
uint64_t DataBuf::read_uint64(size_t offset, ByteOrder byteOrder) const {
  if (pData_.size() < 8 || offset > (pData_.size() - 8)) {
    ...
    return 0;
    // throw std::out_of_range("Overflow in DataBuf::read_uint64");
  }
...
void DataBuf::write_uint64(size_t offset, uint64_t x, ByteOrder byteOrder) {
  if (pData_.size() < 8 || offset > (pData_.size() - 8)) {
    ...
    return;
    // throw std::out_of_range("Overflow in DataBuf::write_uint64");
  }
...
bool stringTo<bool>(const std::string& s, bool& ok) {
  if (s.empty())
    return false;
  auto lcs = lower(s); /* lowercase string */
