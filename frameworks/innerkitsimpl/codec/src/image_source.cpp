...
std::unique_ptr<Picture> ImageSource::CreatePicture(const DecodingOptionsForPicture &opts, uint32_t &errorCode)
{
    ...
    if (format != IMAGE_HEIF_FORMAT && format != IMAGE_JPEG_FORMAT) {
        IMAGE_LOGE("[ImageSource]CreatePicture failed, unsupport format: %{public}s", format.c_str(););
        return nullptr;
    }
    ...
    const std::set<AuxiliaryPictureType> &auxTypes, std::unique_ptr<Picture> &picture, uint32_t &errorCode)
{
    ...
    if (!jpegMpfParser->CheckMpfOffset(streamBuffer, streamSize, mpfOffset)) {
        IMAGE_LOGE("Jpeg CheckMpfOffset failed! mpfOffset: %{public}u", mpfOffset);
    ...
    for (auto &auxInfo : jpegMpfParser->images_) {
        ...
        if (iter != auxTypes.end()) {
            IMAGE_LOGI("Jpeg auxiliary picture has found. Type: %{public}d", static_cast<int32_t>(auxInfo.auxType));
            uint32_t auxOffset = mpfOffset + JPEG_MPF_IDENTIFIER_SIZE + auxInfo.offset;
            std::unique_ptr<InputDataStream> auxStream =
                BufferSourceStream::CreateSourceStream((streamBuffer + auxOffset), auxInfo.size);
            if (auxStream == nullptr) {
                IMAGE_LOGE("Create auxiliary stream fail, auxiliary offset is %{public}u", auxOffset);
                break;
            }
