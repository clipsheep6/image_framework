...
void PixelYuv::rotate(float degrees)
{
    ...
    if (!PixelYuvUtils::YuvRotate(data_, imageInfo_.size, dst, degrees, imageInfo_.pixelFormat)) {
        dstMemory->Release();
        return;
    }
...
#endif
    if (!PixelYuvUtils::YuvCrop(data_, imageInfo_.size, (uint8_t *)dstMemory->data.data, rect, imageInfo.pixelFormat)) {
        dstMemory->Release();
        return ERR_IMAGE_CROP;
    }
...
#endif
        uint8_t *argbData = reinterpret_cast<uint8_t *>(dstMemory->data.data);
        if (!PixelYuvUtils::Yuv420ToBGRA(data_, argbData, imageInfo.size.width,
            imageInfo.size.height, imageInfo.pixelFormat)) {
            dstMemory->Release();
            return ERR_IMAGE_CROP;
        }
        format = imageInfo.pixelFormat;
        imageInfo.pixelFormat = PixelFormat::BGRA_8888;
        SetPixelsAddr(argbData, nullptr, pictureSize, dstMemory->GetType(), nullptr);
    }
    return SUCCESS;
}
...
int32_t PixelYuv::ScaleBGRAToYuv(ImageInfo &imageInfo, PixelFormat &format)
{
    if (format != PixelFormat::UNKNOWN) {
        ...
        if (!PixelYuvUtils::BGRAToYuv420(data_, yuvData, imageInfo.size.width, imageInfo.size.height, format)) {
            dstMemory->Release();
            return false;
        }
        SetPixelsAddr(yuvData, nullptr, pictureSize, dstMemory->GetType(), nullptr);
...
bool PixelYuv::FlipXaxis(const uint8_t *src, uint8_t *dst, int32_t srcW, int32_t srcH, PixelFormat format)
{
    uint32_t dstSize = GetImageSize(srcW, srcH);
    switch (format) {
        case PixelFormat::YU12:
        case PixelFormat::YV12:
            PixelYuvUtils::I420Copy(src, srcW, src + GetYSize(srcW, srcH), GetYSize(srcW, srcH),
        ...
        case PixelFormat::NV21:
            PixelYuvUtils::NV12Copy(src, srcW, src + GetYSize(srcW, srcH), GetUVStride(srcW),
    ...
    if (memcpy_s(data_, dstSize, dst, dstSize) != 0) {
        IMAGE_LOGE("FlipXaxis memcpy failed");
        return false;
    }
    return true;
}

bool PixelYuv::FlipYaxis(const uint8_t *src, uint8_t *dst, int32_t srcW, int32_t srcH, PixelFormat format)
{
    uint32_t dstSize = GetImageSize(srcW, srcH);
    switch (format) {
        case PixelFormat::YU12:
        case PixelFormat::YV12:
            PixelYuvUtils::I420Mirror(src, srcW, src + GetYSize(srcW, srcH), GetUStride(srcW),
        ...
        case PixelFormat::NV12:
        case PixelFormat::NV21:
            PixelYuvUtils::NV12Mirror(src, srcW, src + GetYSize(srcW, srcH), GetUVStride(srcW),
            dst, srcW, dst + GetYSize(srcW, srcH), GetUVStride(srcW), srcW, srcH);
            break;
        default:
            break;
    ...
    if (memcpy_s(data_, dstSize, dst, dstSize) != 0) {
        IMAGE_LOGE("FlipYaxis memcpy failed");
        return false;
    }
    return true;
}
...
void PixelYuv::flip(bool xAxis, bool yAxis)
{
    ...
    uint8_t *dst = reinterpret_cast<uint8_t *>(dstMemory->data.data);
    if (xAxis) {
        if (!FlipXaxis(src, dst, srcW, srcH, format)) {
            IMAGE_LOGE("flip xAxis failed");
            return;
        }
    }
    if (yAxis) {
        if (!FlipYaxis(src, dst, srcW, srcH, format)) {
            IMAGE_LOGE("flip yAxis failed");
            return;
        }
    }
...
void PixelYuv::translate(float xAxis, float yAxis)
{
    ...
    if (!PixelYuvUtils::YuvTranslate(data_, imageInfo_.size, dst, imageInfo_.pixelFormat, xAxis, yAxis)) {
        dstMemory->Release();
        return;
    }
...
ColorYuv420 PixelYuv::GetYuv420Color(uint32_t x, uint32_t y)
{
    ...
    colorYuv.colorY = PixelYuvUtils::GetYuv420Y(x, y, width, data_);
    colorYuv.colorU = PixelYuvUtils::GetYuv420U(x, y, width, height, format, data_);
    colorYuv.colorV = PixelYuvUtils::GetYuv420V(x, y, width, height, format, data_);
    return colorYuv;
}
