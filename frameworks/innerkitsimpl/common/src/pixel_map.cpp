    ...
    AllocatorType &allocatorType, int32_t &bufferSize, PIXEL_MAP_ERR &error)
{
    ...
    if (bytesPerPixel == 0) {
        HiLog::Error(LABEL, "ReadPropertiesFromParcel bytesPerPixel fail");
        return false;
    }
    if ((!isAstc) && bufferSize != rowDataSize * imgInfo.size.height) {
        HiLog::Error(LABEL, "ReadPropertiesFromParcel bufferSize invalid");
    ...
    if (pixelMemInfo.allocatorType == AllocatorType::SHARE_MEM_ALLOC) {
        ...
        *static_cast<int32_t *>(pixelMemInfo.context) = fd;
        ...
        if (pixelMemInfo.base == nullptr) {
            HiLog::Error(LABEL, "get pixel memory size:[%{public}d] error.", pixelMemInfo.bufferSize);
...
PixelMap *PixelMap::Unmarshalling(Parcel &parcel, PIXEL_MAP_ERR &error)
{
    ...
    if (!UpdatePixelMapMemInfo(pixelMap, imgInfo, pixelMemInfo)) {
        HiLog::Error(LABEL, "update pixelMap memInfo fail");
        delete pixelMap;
        return nullptr;
    }
