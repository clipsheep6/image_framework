                                    ...
                                    const Rect &srcRect, PixelMap &dstPixelMap)
{
    ...
    if (dstPixels == nullptr) {
        HiLog::Error(LABEL, "allocate memory size %{public}u fail", bufferSize);
        dstPixels = calloc(bufferSize, 1);
        if (dstPixels == nullptr) {
            HiLog::Error(LABEL, "calloc memory size %{public}u fail", bufferSize);
            return false;
        }
    }
...
bool PixelMap::CopyPixelMap(PixelMap &source, PixelMap &dstPixelMap)
{
    ...
    void *dstPixels = AllocSharedMemory(bufferSize, fd);
    if (dstPixels == nullptr) {
        HiLog::Error(LABEL, "allocate shared memory fail");
        void *dstPixels = malloc(bufferSize);
        if (dstPixels == nullptr) {
            HiLog::Error(LABEL, "allocate heap memory fail");
            return false;
        }
    }
