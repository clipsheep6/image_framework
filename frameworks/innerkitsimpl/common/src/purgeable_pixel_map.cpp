...
namespace OHOS {
namespace Media {
using namespace OHOS::PurgeableMem;
using namespace OHOS::HiviewDFX;
using namespace std;
constexpr OHOS::HiviewDFX::HiLogLabel LABEL = { LOG_CORE, LOG_TAG_DOMAIN_ID_IMAGE, "PurgeablePixelMap" };
...
// for create2
class PixelMapBuilder2 : public PurgeableMemBuilder {
public:
    PixelMapBuilder2(const uint32_t *colors, uint32_t offset, uint32_t stride, int32_t dstrowDataSize, const ImageInfo &srcImageInfo, const ImageInfo &dstImageInfo)
    {
        std::cout << "PixelMapBuilder2 create" << std::endl;
        this->colors_ = colors; // *colors* may have been freed when used in build()
        this->offset_ = offset;
        this->stride_ = stride;
        this->dstrowDataSize_ = dstrowDataSize;
        this->srcImageInfo_ = srcImageInfo;
        this->dstImageInfo_ = dstImageInfo;
    }

    bool Build(void *data, size_t size)
    {
        Position dstPosition;
        std::cout << "PixelMapBuilder2 rebuild!" << std::endl;
        if (!PixelConvertAdapter::WritePixelsConvert(reinterpret_cast<const void *>(colors_ + offset_),
            static_cast<uint32_t>(stride_) << FOUR_BYTE_SHIFT, srcImageInfo_,
            data, dstPosition, dstrowDataSize_, dstImageInfo_)) {
            std::cout << "PixelMapBuilder2 failed!" << std::endl;
            HiLog::Error(LABEL, "PixelMapBuilder2: pixel convert in adapter failed.");
            return false;
        }
        std::cout << "PixelMapBuilder2 success!" << std::endl;
        return true;
    }

    ~PixelMapBuilder2()
    {
        std::cout << "~PixelMapBuilder2" << std::endl;
    }

private:
    const uint32_t *colors_ = nullptr;
    uint32_t offset_;
    uint32_t stride_;
    int32_t dstrowDataSize_;
    ImageInfo srcImageInfo_;
    ImageInfo dstImageInfo_;
};

// for create3
class PixelMapBuilder3 : public PurgeableMemBuilder {
public:
    PixelMapBuilder3(const AlphaType &alphaType, const PixelFormat &pixelFormat, uint8_t pixelBytes, uint32_t bufferSize)
    {
        std::cout << "PixelMapBuilder3 create" << std::endl;
        this->alphaType_ = alphaType;
        this->pixelFormat_ = pixelFormat;
        this->pixelBytes_ = pixelBytes;
        this->bufferSize_ = bufferSize;
    }

    bool Build(void *data, size_t size)
    {
        if (bufferSize_ > static_cast<uint32_t>(size)) {
            HiLog::Error(LABEL, "PixelMapBuilder3: buffersize > build size");
            return false;
        }

        std::cout << "PixelMapBuilder3 rebuild" << std::endl;
        uint8_t *dstPixels = static_cast<uint8_t *>(data);
        if (alphaType_ == AlphaType::IMAGE_ALPHA_TYPE_OPAQUE) {
            int8_t alphaIndex = -1;
            if (pixelFormat_ == PixelFormat::RGBA_8888 || pixelFormat_ == PixelFormat::BGRA_8888) {
                alphaIndex = BGRA_ALPHA_INDEX;
            } else if (pixelFormat_ == PixelFormat::ARGB_8888) {
                alphaIndex = 0;
            }
            if (alphaIndex != -1) {
        ...
        std::cout << "PixelMapBuilder3 success!" << std::endl;
        return true;
    }

    ~PixelMapBuilder3()
    {
        std::cout << "~PixelMapBuilder3" << std::endl;
    }
...
PurgeablePixelMap::~PurgeablePixelMap()
{
    FreePurgeablePixelMap();
}

void PurgeablePixelMap::FreePurgeablePixelMap()
{
    if (data_ == nullptr) {
        return;
    }
    switch (allocatorType_) {
        ...
        case AllocatorType::CUSTOM_ALLOC: {
            if (custFreePurgeablePixelMap_ != nullptr) {
                custFreePurgeablePixelMap_(data_, context_, pixelsSize_);
            }
            delete data_;
            data_ = nullptr;
            context_ = nullptr;
        ...
        case AllocatorType::SHARE_MEM_ALLOC: {
            ReleaseSharedMemory(data_->GetContent(), context_, pixelsSize_);
            delete data_;
            data_ = nullptr;
            context_ = nullptr;
            break;
...
void PurgeablePixelMap::ReleaseSharedMemory(void *addr, void *context, uint32_t size)
{
#if !defined(_WIN32) && !defined(_APPLE)
...
void PurgeablePixelMap::SetPixelsAddr(void *addr, void *context, uint32_t size, AllocatorType type, CustomFreePurgeablePixelMap func)
{
    FreePurgeablePixelMap();
    data_ = static_cast<OHOS::PurgeableMem::PurgeableMem *>(addr);
    context_ = context;
    pixelsSize_ = size;
    allocatorType_ = type;
    custFreePurgeablePixelMap_ = func;
}

unique_ptr<PurgeablePixelMap> PurgeablePixelMap::Create(const uint32_t *colors, uint32_t colorLength, const InitializationOptions &opts)
{
    HiLog::Info(LABEL, "PurgeablePixelMap::Create1 enter");
    return Create(colors, colorLength, 0, opts.size.width, opts);
}

unique_ptr<PurgeablePixelMap> PurgeablePixelMap::Create(const uint32_t *colors, uint32_t colorLength, int32_t offset, int32_t stride,
                                      const InitializationOptions &opts)
{
    HiLog::Info(LABEL, "PurgeablePixelMap::Create2 enter");
    if (!CheckParams(colors, colorLength, offset, stride, opts)) {
        return nullptr;
    }
    unique_ptr<PurgeablePixelMap> dstPixelMap = make_unique<PurgeablePixelMap>();
    ...
    std::unique_ptr<PurgeableMemBuilder> builder2 =
        std::make_unique<PixelMapBuilder2>(colors, offset, stride, dstPixelMap->GetRowBytes(), srcImageInfo, dstImageInfo);
    OHOS::PurgeableMem::PurgeableMem *dstPixels = new OHOS::PurgeableMem::PurgeableMem(bufferSize, std::move(builder2));
    ...
    //     dstPixels->GetContent(), dstPosition, dstPixelMap->GetRowBytes(), dstImageInfo)) {
    ...
    std::cout << "PixelMapCreate2 success!" << std::endl;
    return dstPixelMap;
}

bool PurgeablePixelMap::CheckParams(const uint32_t *colors, uint32_t colorLength, int32_t offset, int32_t stride,
                           const InitializationOptions &opts)
{
...
unique_ptr<PurgeablePixelMap> PurgeablePixelMap::Create(const InitializationOptions &opts)
{
    HiLog::Info(LABEL, "PurgeablePixelMap::Create3 enter");
    unique_ptr<PurgeablePixelMap> dstPixelMap = make_unique<PurgeablePixelMap>();
    if (dstPixelMap == nullptr) {
        HiLog::Error(LABEL, "create pixelMap pointer fail");
        return nullptr;
    ...
    std::unique_ptr<PurgeableMemBuilder> builder3 =
        std::make_unique<PixelMapBuilder3>(dstImageInfo.alphaType, dstImageInfo.pixelFormat, dstPixelMap->GetPixelBytes(), dstPixelMap->GetByteCount());
    OHOS::PurgeableMem::PurgeableMem *dstPixels = new OHOS::PurgeableMem::PurgeableMem(bufferSize, std::move(builder3));

    // // update alpha opaque
    // UpdatePixelsAlpha(dstImageInfo.alphaType, dstImageInfo.pixelFormat, static_cast<uint8_t *>(dstPixels->GetContent()), *dstPixelMap.get());
    dstPixelMap->SetPixelsAddr(dstPixels, nullptr, bufferSize, AllocatorType::HEAP_ALLOC, nullptr);
    dstPixelMap->SetEditable(opts.editable);
    std::cout << "PixelMapCreate3 success!" << std::endl;
    return dstPixelMap;
}

void PurgeablePixelMap::UpdatePixelsAlpha(const AlphaType &alphaType, const PixelFormat &pixelFormat, uint8_t *dstPixels,
                                 PurgeablePixelMap dstPixelMap)
{
...
/* not supported yet
unique_ptr<PurgeablePixelMap> PurgeablePixelMap::Create(PurgeablePixelMap &source, const InitializationOptions &opts)
{
    HiLog::Info(LABEL, "PurgeablePixelMap::Create4 enter");
    Rect rect;
    return Create(source, rect, opts);
}

unique_ptr<PurgeablePixelMap> PurgeablePixelMap::Create(PurgeablePixelMap &source, const Rect &srcRect, const InitializationOptions &opts)
{
    HiLog::Info(LABEL, "PurgeablePixelMap::Create5 enter");
    ImageInfo srcImageInfo;
    ...
    if (opts.useSourceIfMatch && !source.IsEditable() && !opts.editable && (cropType == CropValue::NOCROP) &&
        ...
        return unique_ptr<PurgeablePixelMap>(&source);
    }
    unique_ptr<PurgeablePixelMap> dstPixelMap = make_unique<PurgeablePixelMap>();
    if (dstPixelMap == nullptr) {
        HiLog::Error(LABEL, "create pixelmap pointer fail");
    ...
    if ((cropType == CropValue::VALID) || isHasConvert) {
        ...
        if (!CopyPurgeablePixelMap(source, *dstPixelMap.get())) {
            return nullptr;
        }
    }
    if (!ScalePurgeablePixelMap(targetSize, dstImageInfo.size, opts.scaleMode, *dstPixelMap.get())) {
        return nullptr;
    }
    dstPixelMap->SetEditable(opts.editable);
    return dstPixelMap;
...
bool PurgeablePixelMap::SourceCropAndConvert(PurgeablePixelMap &source, const ImageInfo &srcImageInfo, const ImageInfo &dstImageInfo,
                                    const Rect &srcRect, PurgeablePixelMap &dstPixelMap)
{
    uint32_t bufferSize = dstPixelMap.GetByteCount();
    if (bufferSize == 0 || bufferSize > PIXEL_MAP_MAX_RAM_SIZE) {
...
bool PurgeablePixelMap::ScalePurgeablePixelMap(const Size &targetSize, const Size &dstSize, const ScaleMode &scaleMode,
                             PurgeablePixelMap &dstPixelMap)
{
    ...
    if (scaleMode == ScaleMode::FIT_TARGET_SIZE) {
        if (!postProc.ScalePurgeablePixelMap(targetSize, dstPixelMap)) {
...
void PurgeablePixelMap::InitDstImageInfo(const InitializationOptions &opts, const ImageInfo &srcImageInfo,
                                ImageInfo &dstImageInfo)
{
...
/* not supported yet
bool PurgeablePixelMap::CopyPurgeablePixelMap(PurgeablePixelMap &source, PurgeablePixelMap &dstPixelMap)
{
...
bool PurgeablePixelMap::IsSameSize(const Size &src, const Size &dst)
{
    return (src.width == dst.width) && (src.height == dst.height);
}

bool PurgeablePixelMap::GetPixelFormatDetail(const PixelFormat format)
{
    switch (format) {
...
uint32_t PurgeablePixelMap::SetImageInfo(ImageInfo &info)
{
    return SetImageInfo(info, false);
}

uint32_t PurgeablePixelMap::SetImageInfo(ImageInfo &info, bool isReused)
{
...
const uint8_t *PurgeablePixelMap::GetPixel8(int32_t x, int32_t y)
{
    ...
    return (static_cast<uint8_t *>(data_->GetContent()) + y * rowDataSize_ + x);
}

const uint16_t *PurgeablePixelMap::GetPixel16(int32_t x, int32_t y)
{
    ...
    return reinterpret_cast<uint16_t *>(static_cast<uint8_t *>(data_->GetContent()) + y * rowDataSize_ + (static_cast<uint32_t>(x) << RGB_565_SHIFT));
}

const uint32_t *PurgeablePixelMap::GetPixel32(int32_t x, int32_t y)
{
    ...
    return reinterpret_cast<uint32_t *>(static_cast<uint8_t *>(data_->GetContent()) + y * rowDataSize_ + (static_cast<uint32_t>(x) << ARGB_8888_SHIFT));
}

const uint8_t *PurgeablePixelMap::GetPixel(int32_t x, int32_t y)
{
    if (!CheckValidParam(x, y)) {
        HiLog::Error(LABEL, "input pixel position:(%{public}d, %{public}d) invalid.", x, y);
        return nullptr;
    }
    return (static_cast<uint8_t *>(data_->GetContent()) + y * rowDataSize_ + (static_cast<uint32_t>(x) * pixelBytes_));
}

bool PurgeablePixelMap::BeginRead()
{
    return data_->BeginRead();
}

void PurgeablePixelMap::EndRead()
{
    data_->EndRead();
}

bool PurgeablePixelMap::BeginWrite()
{
    return data_->BeginWrite();
}

void PurgeablePixelMap::EndWrite()
{
    data_->EndWrite();
}

bool PurgeablePixelMap::GetARGB32Color(int32_t x, int32_t y, uint32_t &color)
{
...
bool PurgeablePixelMap::ALPHA8ToARGB(const uint8_t *in, uint32_t inCount, uint32_t *out, uint32_t outCount)
{
    if (inCount != outCount) {
...
bool PurgeablePixelMap::RGB565ToARGB(const uint8_t *in, uint32_t inCount, uint32_t *out, uint32_t outCount)
{
...
bool PurgeablePixelMap::ARGB8888ToARGB(const uint8_t *in, uint32_t inCount, uint32_t *out, uint32_t outCount)
{
...
bool PurgeablePixelMap::RGBA8888ToARGB(const uint8_t *in, uint32_t inCount, uint32_t *out, uint32_t outCount)
{
    if (((inCount / ARGB_8888_BYTES) != outCount) && ((inCount % ARGB_8888_BYTES) != 0)) {
        HiLog::Error(LABEL, "input count:%{public}u is not match to output count:%{public}u.", inCount, outCount);
        return false;
...
bool PurgeablePixelMap::BGRA8888ToARGB(const uint8_t *in, uint32_t inCount, uint32_t *out, uint32_t outCount)
{
    if (((inCount / ARGB_8888_BYTES) != outCount) && ((inCount % ARGB_8888_BYTES) != 0)) {
        HiLog::Error(LABEL, "input count:%{public}u is not match to output count:%{public}u.", inCount, outCount);
        return false;
...
bool PurgeablePixelMap::RGB888ToARGB(const uint8_t *in, uint32_t inCount, uint32_t *out, uint32_t outCount)
{
...
int32_t PurgeablePixelMap::GetPixelBytes()
{
    return pixelBytes_;
}

int32_t PurgeablePixelMap::GetRowBytes()
{
    return rowDataSize_;
}

int32_t PurgeablePixelMap::GetByteCount()
{
    HiLog::Debug(LABEL, "GetByteCount");
    return rowDataSize_ * imageInfo_.size.height;
}

int32_t PurgeablePixelMap::GetWidth()
{
    return imageInfo_.size.width;
}

int32_t PurgeablePixelMap::GetHeight()
{
    return imageInfo_.size.height;
}

int32_t PurgeablePixelMap::GetBaseDensity()
{
    return imageInfo_.baseDensity;
}

void PurgeablePixelMap::GetImageInfo(ImageInfo &imageInfo)
{
    imageInfo = imageInfo_;
}

PixelFormat PurgeablePixelMap::GetPixelFormat()
{
    return imageInfo_.pixelFormat;
}

ColorSpace PurgeablePixelMap::GetColorSpace()
{
    return imageInfo_.colorSpace;
}

AlphaType PurgeablePixelMap::GetAlphaType()
{
    return imageInfo_.alphaType;
}

const uint8_t *PurgeablePixelMap::GetPixels()
{
    return static_cast<uint8_t *>(data_->GetContent());
}

uint8_t PurgeablePixelMap::GetARGB32ColorA(uint32_t color)
{
    return (color >> ARGB_A_SHIFT) & ARGB_MASK;
}

uint8_t PurgeablePixelMap::GetARGB32ColorR(uint32_t color)
{
    return (color >> ARGB_R_SHIFT) & ARGB_MASK;
}

uint8_t PurgeablePixelMap::GetARGB32ColorG(uint32_t color)
{
    return (color >> ARGB_G_SHIFT) & ARGB_MASK;
}

uint8_t PurgeablePixelMap::GetARGB32ColorB(uint32_t color)
{
    return (color >> ARGB_B_SHIFT) & ARGB_MASK;
}

bool PurgeablePixelMap::IsSameImage(const PurgeablePixelMap &other)
{
    if (data_ == nullptr || other.data_ == nullptr || data_->GetContent() == nullptr || other.data_->GetContent() == nullptr) {
        HiLog::Error(LABEL, "IsSameImage data_ or data_->GetContent() is nullptr.");
        return false;
    }
    ...
    if (memcmp(data_->GetContent(), other.data_->GetContent(), size) != 0) {
...
uint32_t PurgeablePixelMap::ReadPixels(const uint64_t &bufferSize, uint8_t *dst)
{
    ...
    if (data_ == nullptr || data_->GetContent() == nullptr) {
        HiLog::Error(LABEL, "read pixels by buffer current PurgeablePixelMap data is null.");
        return ERR_IMAGE_READ_PIXELMAP_FAILED;
    }
    ...
    errno_t ret = memcpy_s(dst, bufferSize, static_cast<uint8_t *>(data_->GetContent()), pixelsSize_);
    if (ret != 0) {
...
bool PurgeablePixelMap::CheckPixelsInput(const uint8_t *dst, const uint64_t &bufferSize, const uint32_t &offset,
...
uint32_t PurgeablePixelMap::ReadPixels(const uint64_t &bufferSize, const uint32_t &offset, const uint32_t &stride,
                              const Rect &region, uint8_t *dst)
{
    ...
    if (!PixelConvertAdapter::ReadPixelsConvert(data_->GetContent(), srcPosition, rowDataSize_, imageInfo_, dst + offset, stride,
        dstImageInfo)) {
...
uint32_t PurgeablePixelMap::ReadPixel(const Position &pos, uint32_t &dst)
{
    ...
    if (!PixelConvertAdapter::ReadPixelsConvert(data_->GetContent(), srcPosition, rowDataSize_, imageInfo_, &dst, dstRowBytes,
        dstImageInfo)) {
...
uint32_t PurgeablePixelMap::ResetConfig(const Size &size, const PixelFormat &format)
{
...
bool PurgeablePixelMap::SetAlphaType(const AlphaType &alphaType)
{
...
uint32_t PurgeablePixelMap::WritePixel(const Position &pos, const uint32_t &color)
{
    if (pos.x < 0 || pos.y < 0 || pos.x >= GetWidth() || pos.y >= GetHeight()) {
        HiLog::Error(LABEL,
    ...
    if (!PixelConvertAdapter::WritePixelsConvert(&color, srcRowBytes, srcImageInfo, data_->GetContent(), dstPosition, rowDataSize_,
        imageInfo_)) {
...
uint32_t PurgeablePixelMap::WritePixels(const uint8_t *source, const uint64_t &bufferSize, const uint32_t &offset,
                               const uint32_t &stride, const Rect &region)
{
    ...
    ImageInfo srcInfo =
        MakeImageInfo(region.width, region.height, PixelFormat::BGRA_8888, AlphaType::IMAGE_ALPHA_TYPE_UNPREMUL);
    if (!PixelConvertAdapter::WritePixelsConvert(source + offset, stride, srcInfo, data_->GetContent(), dstPosition, rowDataSize_,
        imageInfo_)) {
...
uint32_t PurgeablePixelMap::WritePixels(const uint8_t *source, const uint64_t &bufferSize)
{
    ...
    errno_t ret = memcpy_s(data_->GetContent(), pixelsSize_, source, pixelsSize_);
    if (ret != 0) {
...
bool PurgeablePixelMap::WritePixels(const uint32_t &color)
{
    if (!IsEditable()) {
    ...
    if (!PixelConvertAdapter::EraseBitmap(data_->GetContent(), rowDataSize_, srcInfo, color)) {
...
AllocatorType PurgeablePixelMap::GetAllocatorType()
{
    return allocatorType_;
}

void *PurgeablePixelMap::GetFd() const
{
    return context_;
}

void PurgeablePixelMap::ReleaseMemory(AllocatorType allocType, void *addr, void *context, uint32_t size)
{
...
bool PurgeablePixelMap::WriteImageData(Parcel &parcel, size_t size) const
{
    if (data_ == nullptr) {
        HiLog::Error(LABEL, "write to parcel failed, purgeable memory is null.");
        return false;
    }

    const uint8_t *data = static_cast<uint8_t *>(data_->GetContent());
    if (data == nullptr) {
...
/* not supported yet
uint8_t *PurgeablePixelMap::ReadImageData(Parcel &parcel, int32_t bufferSize)
{
...
bool PurgeablePixelMap::WriteFileDescriptor(Parcel &parcel, int fd)
{
    if (fd < 0) {
        return false;
    }
...
int PurgeablePixelMap::ReadFileDescriptor(Parcel &parcel)
{
...
bool PurgeablePixelMap::WriteImageInfo(Parcel &parcel) const
{
...
bool PurgeablePixelMap::Marshalling(Parcel &parcel) const
{
...
bool PurgeablePixelMap::ReadImageInfo(Parcel &parcel, ImageInfo &imgInfo)
{
...
/* not supported yet
PurgeablePixelMap *PurgeablePixelMap::Unmarshalling(Parcel &parcel)
{
    PurgeablePixelMap *pixelMap = new PurgeablePixelMap();
    if (pixelMap == nullptr) {
        return nullptr;
    }
...
uint32_t PurgeablePixelMap::SetAlpha(const float percent)
{
    ...
    return DoSetAlpha(pixelFormat, static_cast<uint8_t *>(data_->GetContent()),
        GetByteCount(), pixelBytes_,
        alphaIndex, percent, pixelPremul);
}

/* not support yet
void PurgeablePixelMap::scale(float xAxis, float yAxis)
{
    PostProc postProc;
...
void PurgeablePixelMap::translate(float xAxis, float yAxis)
{
    PostProc postProc;
...
void PurgeablePixelMap::rotate(float degrees)
{
    PostProc postProc;
...
void PurgeablePixelMap::flip(bool xAxis, bool yAxis)
{
...
uint32_t PurgeablePixelMap::crop(const Rect &rect)
{
    PostProc postProc;
    ...
    void PurgeablePixelMap::InnerSetColorSpace(const OHOS::ColorManager::ColorSpace &grColorSpace)
    {
        grColorSpace_ = std::make_shared<OHOS::ColorManager::ColorSpace>(grColorSpace.ToSkColorSpace());
    }

    OHOS::ColorManager::ColorSpace PurgeablePixelMap::InnerGetGrColorSpace()
    {
