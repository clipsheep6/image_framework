...
MIDK_EXPORT
Image_ErrorCode OH_PackingOptions_NeedExif(OH_PackingOptions *options, bool needExif)
{
    if (options == nullptr) {
        return IMAGE_BAD_PARAMETER;
    }
    options->needExif = static_cast<bool>(needExif);
    return IMAGE_SUCCESS;
}
    ...
    OH_PackingOptions *options, OH_ImageSourceNative *imageSource, uint8_t *outData, size_t *size)
{
    ...
    packOption.needExif = options->needExif;
