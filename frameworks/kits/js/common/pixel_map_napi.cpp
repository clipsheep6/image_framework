...
STATIC_EXEC_FUNC(CreatePremultipliedPixelMap)
{
    ...
    if (IMG_NOT_NULL(context->rPixelMap) && IMG_NOT_NULL(context->wPixelMap)) {
        context->wPixelMap->GetImageInfo(dstImageInfo);
        bool isPremul = true;
        if (context->wPixelMap->IsEditable()) {
            context->status = context->rPixelMap->ConvertAlphaFormat(dstImageInfo, context->wPixelMap->GetCapacity(),
                context->wPixelMap->GetPixelBytes(), context->wPixelMap->GetWritablePixels(), isPremul);
...
{
    IMAGE_LOGE("## whl ## Enter createUnpre");
    napi_value result = nullptr;
    napi_get_undefined(env, &result);
    int32_t refCount = 1;
    napi_status status;
