...
napi_value ImagePackerNapi::PackingFromPixelMap(napi_env env, napi_callback_info info)
{
    ...
    for (size_t i = PARAM0; i < argc; i++) {
        napi_valuetype argvType = ImageNapiUtils::getType(env, argv[i]);
        if (i == PARAM0 && argvType == napi_object) {
            std::shared_ptr<PixelMapNapi> pixelMapNapi = std::make_unique<PixelMapNapi>();
            status = napi_unwrap(env, argv[i], reinterpret_cast<void**>(&pixelMapNapi));
            IMG_NAPI_CHECK_RET_D(IMG_IS_READY(status, pixelMapNapi), nullptr,
                HiLog::Error(LABEL, "fail to unwrap pixelMapNapi"));
            asyncContext->rPixelMap = pixelMapNapi->nativePixelMap_;
