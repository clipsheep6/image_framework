...
napi_value ImagePackerNapi::Init(napi_env env, napi_value exports)
{
    napi_property_descriptor props[] = {
        ...
        DECLARE_NAPI_FUNCTION("packToDataMultiFrames", PackingMultiFrames),
...
static bool parsePackOptionOfdelayTimes(napi_env env, napi_value root, PackOption* opts)
{
    ...
    if (isDelayTimesArray) {
        ...
        for (size_t i = 0; i < len; i++) {
            ...
            opts->delayTimes.push_back(static_cast<uint16_t>(num & 0xffff));
        }
    }
    return true;
}
...
static bool parsePackOptionOfLoop(napi_env env, napi_value root, PackOption* opts)
{
    ...
    opts->loop = static_cast<uint16_t>(tmpNumber & 0xffff);
